...
HAS_LEGACY_DATA = path.isfile(TEST_DIFF_IM)


@pytest.mark.skipif(not HAS_LEGACY_DATA, reason="Requires legacy 4096x4096 diff image")
...
@pytest.mark.skipif(not HAS_LEGACY_DATA, reason="Requires legacy 4096x4096 diff image")
def test_source_find_write_fits():
    # Test against the catalog generated by Marin's script.
    with tempfile.TemporaryDirectory() as dir:
        shutil.copy(TEST_DIFF_IM, f'{dir}/test.fits')
        out_fits = source_find.sourcefind_multithread(f'{dir}/test.fits',
                                                      (0.47967978178195703, 0.22426370283084199, 49.280091354775998),
                                                      n_proc=1, write_fits=True)
        assert '.fits' in out_fits
        t = table.Table.read(out_fits)
