#!/usr/bin/env bash
# =============================================================================
#  03_image_lower_band.sh — Combined wideband MFS imaging (≤41 MHz)
# =============================================================================
#  Collects one calibrated MS per lower-band subband (same timestep) and
#  passes them all to a single wsclean call for wideband MFS synthesis.
#  Stokes I only, no polarization, no peeling.
#
#  Subbands: 18 23 27 32 36 41 MHz  (13 MHz excluded)
#  Output:   one combined MFS image set (channels-out = number of subbands)
# =============================================================================
set -euo pipefail
...
echo "=============================================="
echo " QA-Light: Combined MFS Imaging (Lower Band)"
echo " Subbands: ${LOWER_SUBBANDS[*]}"
...
if [[ ${N_BANDS} -eq 0 ]]; then
    echo "[ERROR] No MS files found for any lower-band subband — aborting"
    exit 1
fi
...
IMG_NAME="${OUTPUT_DIR}/lower-band-QA-MFS"
...
if "${CMD[@]}"; then
    echo ""
    echo "=============================================="
    echo " Lower-band combined MFS imaging complete"
