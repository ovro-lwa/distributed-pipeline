from orca.utils.coordutils import OVRO_LWA_LOCATION, is_visible, get_altaz_at_ovro
...
def gen_model_ms_stokes(ms: str, zest: bool = False):
    ...
    for s,src in enumerate(cal_srcs):
        ...
        if is_visible(SkyCoord(ra, dec, frame=ICRS), utctime):
            ...
            cal_srcs[s]['flux'] = str(list(flux80_47(float(src['flux']), src['alpha'], freq, src['ref_freq']) * scale))
        else:
            del cal_srcs[s]
    
    cl = componentlist()
    if zest:
        fluxIvals  = [float(src['flux'].replace('[','').replace(']','').split(',')[0]) for src in cal_srcs]
        ...
        for ind in sortedinds:
            src     = cal_srcs[ind]
            cl.done()
            cl.addcomponent(flux=eval(src['flux']), polarization='Stokes', dir=src['position'], label=src['label'])
