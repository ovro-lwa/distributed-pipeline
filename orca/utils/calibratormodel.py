...
from orca.utils.modelutils import (
    get_sun_pos,
    get_solar_altaz_multiple_times,
    get_solar_azel,
    get_msinfo,
    get_image_data,
    get_image_maxmin,
    check_image_quality,
    restore_flag,
    get_flagged_solution_num,
    get_strong_source_list,
    get_time_from_name,
    get_timestr_from_name,
    get_freqstr_from_name,
    get_selfcal_time_to_apply,
    get_keyword,
    put_keyword,
    get_obs_time_interval,
    convert_to_heliocentric_coords,
    make_wsclean_compatible,
    get_total_fields,
    collect_fast_fits,
    rename_images,
    check_corrected_data_present,
    correct_primary_beam,
    get_solar_loc_pix,
    get_rms,
    blank_all_pixels,
    get_uvlambda_from_uvdist,
    swap_fastms_pols,
    correct_fastms_amplitude_scale,
    compress_fits_to_h5,
    recover_fits_from_h5,
    check_h5_fits_consistency,
    check_for_file_presence,
    manual_split_corrected_ms,
    get_primary_beam_single_source,
    get_caltable_freq
)
...
class model_generation():
    ...
    def point_source_model(self, included_sources=None):
        srcs=self.get_risen_source_list()                  
        if included_sources:
            srcs = [s for s in srcs if s["label"] in included_sources]
    ...
    def check_negative_in_model(self):
        if self.pol=='I':
            max1, min1 = get_image_maxmin(self.outpath+"calibrator-model.fits", local=False)
            ...
            for pola in self.polarisations:
                max1, min1 = get_image_maxmin(self.outpath+"calibrator-"+pola+"-model.fits", local=False)
                if min1 < 0 and (max1 / max(abs(min1), 0.000001)) < 10000:  ### some small negative is tolerable
                    raise RuntimeError("Negative in model. Going for point source model")
    ...
    def gen_model_cl(self, included_sources=None):
        """
        ...
        if self.point_source_model_needed==True:
           modelcl, ft_needed = self.point_source_model(included_sources)
