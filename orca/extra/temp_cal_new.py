...
freq_subdirs = sorted(glob.glob('/lustre/pipeline/slow/*'))
...
def make_cal_tables(prefix: str, refant: str, image: bool):
    ...
    hour = prefix[9:11]
    inds = [node*2 - 1, node*2 - 2]
    ms_l = []
    for i in inds:
        sd = freq_subdirs[i]
        p = glob.glob(os.path.join(sd,date,hour,f'{prefix}*'))[0]
        isdir = os.path.isdir(p)
        if not isdir:
            logger.warning(f'Directory {p} not found.')
        elif isdir:
            ms_l.append(p)
